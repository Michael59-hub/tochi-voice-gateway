...
| Area | Files | Behavior |
|---|---|---|
| Proposal fail-closed | `src/providers/llmProposalProvider.ts`, `src/providers/proposalFallbackProvider.ts`, `src/providers/index.ts`, `src/routes/voice.ts` | Sahara transcript goes to Gemini in `auto`/`llm` proposal mode. Gemini HTTP 503 falls back to Groq and then Mistral if Groq fails. Missing keys, provider exceptions, timeouts, and exhausted fallbacks do **not** return a mock draft. Fixed safe error categories map to 502/504/429. Explicit `VOICE_PROPOSAL_PROVIDER_MODE=mock` is the only canned proposal mode. |
